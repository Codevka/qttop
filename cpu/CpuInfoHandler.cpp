...
CpuInfo &CpuInfoHandler::collectCpuInfo()
{
    ...
    if (ifs.good()) {
        ifs >> cur_cpu.load_avg_1_5_15[0] >> cur_cpu.load_avg_1_5_15[1]
            >> cur_cpu.load_avg_1_5_15[2];
    }
    ifs.close();
    qDebug() << cur_cpu.load_avg_1_5_15[0] << cur_cpu.load_avg_1_5_15[1]
             << cur_cpu.load_avg_1_5_15[2];
...

    {
        ...
        uint64_t total = std::max((uint64_t) 0,
                                  time_sum
                                      - (times.size() > 8
        ...
        cur_cpu.cpu_percent_user.push_back(
            std::clamp((long long) round((double) (times[0] - old_cpu_user) * 1000 / calc_total),
                       0ll,
                       1000ll));
        qDebug() << "cpu_percent_user: "
                 << std::clamp((long long) round((double) (times[0] - old_cpu_user) * 100
                                                 / calc_total),
                               0ll,
                               1000ll);
        ...
        cur_cpu.cpu_percent_system.push_back(
            std::clamp((long long) round((double) (times[2] - old_cpu_system) * 1000 / calc_total),
                       0ll,
                       1000ll));
        qDebug() << "cpu_percent_system: "
                 << std::clamp((long long) round((double) (times[2] - old_cpu_system) * 1000
                                                 / calc_total),
                               0ll,
                               1000ll);
    ...
    while (i < core_num || (ifs.good() && ifs.peek() == 'c')) {
        ...
        cur_cpu.core_percent[i].push_back(
            std::clamp((long long) round((double) (calc_total - calc_idle) * 100 / calc_total),
                       0ll,
                       100ll));
